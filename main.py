...
def train(walk=False, struc=False, link=False, attr=False, walk_struc=False):
    ...
    pre_train.walk_proximity(trained=walk, num_walks=100, walk_length=25, p=1, q=1)
    pre_train.structure_proximity(trained=struc)
    pre_train.link_proximity(trained=link)
    # pre_train.walk_structure_proximity(trained=walk_struc, num_walks=200, walk_length=5, p=1, q=1)
